...
spread_data <- function(df){
  gc_df <-  df[, c(1, 2, 4)] %>% spread(key = "dilution", value = "OD_N")
  colnames(gc_df)[1] <- "time"
  gc_df$time <- gc_df$time / 60
  return(gc_df)
}

gc_df_s1 <- spread_data(stat.sample1) #growth curver data frame sample 1
gc_df_s2 <- spread_data(stat.sample2) #growth curver data frame sample 2
gc_df_s3 <- spread_data(stat.sample3) #growth curver data frame sample 3
...
#### Sample 1 without bg_correct argument specified (defaults to "min"):

```{r}
gc_out_s1_gb_correct_ns <- SummarizeGrowthByPlate(gc_df_s1, plot_fit = TRUE, 
                                 plot_file = "./plots/gc_plots_s1_gb_correct_not_specified.pdf")
gc_out_s1_gb_correct_ns

```


#### Sample 2 without bg_correct argument specified (defaults to "min"):

```{r}
gc_out_s2_gb_correct_ns <- SummarizeGrowthByPlate(gc_df_s2, plot_fit = TRUE, 
                                 plot_file = "./plots/gc_plots_s2_gb_correct_not_specified.pdf")
gc_out_s2_gb_correct_ns

```

#### Sample 3 without bg_correct argument specified (defaults to "min"):

```{r}
gc_out_s3_gb_correct_ns <- SummarizeGrowthByPlate(gc_df_s3, plot_fit = TRUE, 
                                 plot_file = "./plots/gc_plots_s3_gb_correct_not_specified.pdf")
gc_out_s3_gb_correct_ns
